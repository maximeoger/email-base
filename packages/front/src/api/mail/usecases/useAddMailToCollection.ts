...
export function useAddMailToCollection () {
  ...
  const invalidateGetCollections = () => queryClient.invalidateQueries({ queryKey: ["collection"] })

  const {  mutateAsync, isPending, error, isSuccess  } = useMutation({
    mutationFn: async (body: AddMailToCollectionDto) => router.addMailToCollection(body),
